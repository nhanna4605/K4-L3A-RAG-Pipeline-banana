...
| Evaluation A/B | Chạy A/B dense-only vs hybrid+RRF trên 15 câu golden, phân tích lỗi và hoàn thiện báo cáo kết quả | `group_project/evaluation/RESULT.md`, `phase1_retrieve.py`, `phase2_score.py`, `recompute_context.py`, `rotation.py` | Done |
...
- **Kết quả A/B (15 câu golden, `top_k=5`, threshold 0,51):**
...
- **Phân tích lỗi:** phần lớn khoảng cách đến từ đúng hai câu. Ở G04, cả hai config đều không lấy được Điều 6 khoản 2 điểm c; Config A vẫn trả lời bằng con số "3/30" lấy từ `news_02.md` nên được chấm faithfulness 1,0 và relevance 1,0 dù nội dung sai so với quy định, còn Config B từ chối và bị chấm relevance 0,0 — tức bị phạt vì hành xử đúng. Chỉ G07 là Config B thua thật: RRF đẩy chunk chứa Điều 8 khoản 1 ra khỏi top-5.

- **Lỗi thứ tư đã phát hiện khi đo:** bản tính context metric đầu tiên báo recall 0,20, mâu thuẫn với kết quả kiểm tra retrieval trước đó là 15/15. Nguyên nhân là hàm so khớp lấy mẫu chuỗi cách 5 ký tự nên hai đoạn chứa cùng một câu nhưng lệch offset thì không giao nhau. Sửa sang đo độ phủ bằng cửa sổ trượt tìm trực tiếp, recall về đúng 0,8674.
...
- Hạn chế của phần evaluation: không dùng được ragas vì free tier Gemini giới hạn 20 request/ngày cho mỗi cặp (API key, model), trong khi ragas cần khoảng 300 request cho 30 mẫu. Bộ đánh giá tự viết gộp faithfulness và answer relevance vào một request judge nên tổng chỉ tốn 60 request, nhưng đổi lại không có bộ metric chuẩn của cộng đồng để đối chiếu. Faithfulness đạt 1,0 tuyệt đối ở cả 30 mẫu, cho thấy judge quá dễ dãi; G04 là phản ví dụ trực tiếp. Golden dataset 15 câu cũng là mức tối thiểu nên chênh lệch 0,0205 ở context recall nằm trong khoảng nhiễu.
