...
- **Cấu hình tích hợp theo README hiện tại:** Gemini `gemini-3.6-flash`; embedding local `BAAI/bge-m3` (1024 chiều); chunk 500 ký tự, overlap 50; ChromaDB cosine; threshold nhóm đề xuất `0.51`. Phần tôi mặc định `top_k=5`, threshold `0.3` nếu biến môi trường trống; cần đặt `SCORE_THRESHOLD=0.51` để áp dụng cấu hình nhóm. Code gọi provider với timeout 30 giây.
